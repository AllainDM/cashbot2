...
from aiogram.types import Message  # Используем настоящий класс Message для имитации структуры

# Предположим, что echo_mess находится в main.py
from main import echo_mess

USER_ID = 123456


@pytest.mark.asyncio
@patch('src.main.config')  # Мокируем config
@patch('src.main.crud')  # Мокируем crud
@patch('src.main.split_message')  # Мокируем парсер
async def test_authorized_user_with_sum(mock_split, mock_crud, mock_config):
    ...
    message_mock = AsyncMock(spec=Message, text="100 Еда Обед")
