...
async def test_reply_is_called_when_month_is_missing():
    ...
    # await handler.get_month_report()
...
# Тест отправки ответа через message.reply()
@pytest.mark.asyncio
async def test_successful_report_generation_sends_reply():
    """
    Тест проверяет, что ReportHandler.get_month_report()
    при наличии данных корректно форматирует отчет И ОТПРАВЛЯЕТ его
    пользователю через message.reply().
    """
    # 1. Настройка.
    # Создаем мок-сообщение с указанием месяца
    mock_message = create_mock_message("/report Июль")

    mock_db_conn = Mock()

    # Мок-функция возвращает тестовые данные
    mock_crud_func = AsyncMock(return_value=[
        {"summ": 1100, "category": "Еда"},
        {"summ": 900, "category": "Еда"},
        {"summ": 800, "category": "Продукты"}
    ...
    # 2. Выполнение.
    # Вызываем публичный метод, который должен отправить ответ
    await handler.get_month_report()

    # 3. Проверка.
    # Проверяем, что message.reply был вызван ровно один раз
    mock_message.reply.assert_called_once()

    # Проверяем, что message.reply был вызван именно с ожидаемым текстом
    # Используем .strip() для надежности сравнения
    mock_message.reply.assert_called_with(expected_full_report.strip())
